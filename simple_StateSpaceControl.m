...
Me_est=0.025;Be_est=0.1;
Me_act=0.055;Be_act=0.225;

[A,B,C,D]=updateModel(Me_est,Be_est);
[W,Z,Q,R,Kr]=getMPCMatrices(A,B,C,D,N,dt,Q_,R_);
K=1e3*[0.0217 6.1258];
N_=6.1258e3;
...
for k=1:Tfinal/dt
    ...
    xn=[x(1,k)+normrnd(0,0.025);x(2,k)+normrnd(0,0.025)];
    u(:,k)=N_*rp-K*xn;
    if u(:,k)>u_lim(2)
        u(:,k)=u_lim(2);
    end
...
%     ydt=x(1,k)+normrnd(0,0.025);
%     yddt=1/Me_act*uk-Be_act/Me_act*x(1,k)+normrnd(0,0.025);
%     
